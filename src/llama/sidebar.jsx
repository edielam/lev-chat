...
const RAGSidebar = ({onToggle}) => {
    ...
    const handleModelChange = async (model) => {
        try {
          // Set the model in the backend state
          await invoke('set_model', { modelName: model });
          
          // Verify the model was set by retrieving it
          const currentModel = await invoke('get_selected_model');
          
          // Update local state
          setSelectedModel(model);
          
          // Optional: Add any additional logic after model selection
      ...
      const handleEmbeddingChange = async (model) => {
        try {
          // Set the embedding model in the backend state
          await invoke('set_em_model', { modelName: model });
          
          // Verify the embedding model was set by retrieving it
          const currentEmbeddingModel = await invoke('get_selected_em_model');
          
          // Update local state
          setSelectedEmbedding(model);
          
          // Optional: Add any additional logic after embedding model selection
          console.log('Selected embedding model:', currentEmbeddingModel);
        } catch (error) {
          console.error('Error setting embedding model:', error);
        }
      };
    
      // Load initially selected models on component mount
      useEffect(() => {
        const loadInitialModels = async () => {
          try {
            const initialModel = await invoke('get_selected_model');
            if (initialModel) {
              setSelectedModel(initialModel);
            }
    
            const initialEmbeddingModel = await invoke('get_selected_em_model');
            if (initialEmbeddingModel) {
              setSelectedEmbedding(initialEmbeddingModel);
            }
          } catch (error) {
