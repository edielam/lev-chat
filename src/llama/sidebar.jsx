...
const DownloadStatus = styled.div<{ $status: 'success' | 'error' | 'downloading' }>`
  display: flex;
  align-items: center;
  gap: 8px;
  padding: 8px;
  background-color: ${props => 
    props.$status === 'success' ? '#e6f4ea' : 
    props.$status === 'error' ? '#fce4ec' : 
    '#f1f3f4'
  };
  color: ${props => 
...
// const ModelListTitle = styled.div`
//   padding: 0.75rem;
//   background-color: ${props => props.theme.border};
//   color: ${props => props.theme.textStrong};
//   font-weight: 500;
...
// const NoModelsMessage = styled.div`
//   padding: 0.75rem;
//   color: ${props => props.theme.textMuted};
//   font-size: 0.875rem;
//   text-align: center;
...
embeddingModel: 'https://huggingface.co/CompendiumLabs/bge-large-en-v1.5-gguf/resolve/main/bge-large-en-v1.5-f16.gguf'
  ...
  const handleModelDownload = async (type) => {
    try {
      ...
      console.log('Downloading model:', { url, model_type: type === 'language' ? 'languageModel' : 'embeddingModel' });
      
      await invoke('download_model', { 
        url, 
        modelType: type === 'language' ? 'languageModel' : 'embeddingModel' 
      });
            ...
            <ModelDownloadSection>
              <ModelDownloadGroup>
                ...
                <ModelUrlInput 
                    ...
                    />
                  <DownloadModelButton onClick={() => handleModelDownload('language')}>
                    <Download size={18} />
                  </DownloadModelButton>
                </ModelDownloadContainer>
              </ModelDownloadGroup>
              ...
              <ModelDownloadGroup>
                ...
                <ModelUrlInput 
                    ...
                    />
                  <DownloadModelButton onClick={() => handleModelDownload('embedding')}>
                    <Download size={18} />
                  </DownloadModelButton>
                </ModelDownloadContainer>
              </ModelDownloadGroup>
